...
object Application extends zio.App {
  ...
  private val webApp = Http.collectM[Request] {
    ...
    case Method.GET -> Root / "before" / endTimeStr =>
      ...
      if (endTimeStrOpt.isEmpty) ZIO.succeed(err)
  ...
  private val err = Response.fromHttpError(HttpError.BadRequest("Invalid time, times must be given as unix time, eg: \"1621957740\""))
}
